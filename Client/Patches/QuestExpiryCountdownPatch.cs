...
public class QuestExpiryCountdownPatch : ModulePatch
{
    ...
    protected override MethodBase GetTargetMethod()
    {
        ...
        if (method == null && Plugin.DebugLogging)
        {
            ...
            Plugin.Log.LogDebug($"[TheQuartermaster] Could not find NotesTaskDescription.Show. Available: {string.Join(", ", allShows.Select(m => $"{m.Name}({string.Join(", ", m.GetParameters().Select(p => p.ParameterType.Name))})"))}");
        }
    ...
    public static void Postfix(NotesTaskDescription __instance, QuestClass quest)
    {
        try
        {
            ...
            if (tmpText == null || tmpText.gameObject == null) return;
...
public class LiveCountdownBehaviour : MonoBehaviour
{
    ...
    private void Update()
    {
        try
        {
            if (_tmpText == null || _tmpText.gameObject == null)
            {
        ...
        catch (Exception ex)
        {
            if (Plugin.DebugLogging)
                Plugin.Log.LogDebug($"[TheQuartermaster] LiveCountdownBehaviour Update error: {ex}");
            UnityEngine.Object.DestroyImmediate(this);
        }
    }
    ...
    private void UpdateText()
    {
        ...
        {
            if (Plugin.DebugLogging)
                Plugin.Log.LogDebug($"[TheQuartermaster] LiveCountdownBehaviour UpdateText error: {ex}");
            UnityEngine.Object.DestroyImmediate(this);
        }
    }
}
